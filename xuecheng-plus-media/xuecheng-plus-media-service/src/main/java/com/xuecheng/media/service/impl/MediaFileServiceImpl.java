...
public class MediaFileServiceImpl implements MediaFileService {
    ...
    public UploadFileResultDto uploadFile(Long companyId, UploadFileParamsDto uploadFileParamsDto, String localFilePath, String objectName) {
        ...
        // 上传文件到minio
        boolean result = addMediaFilesToMinion(localFilePath, mimeType, bucket_mediafiles, objectName);
        if (!result) {
        ...
        if (mediaFiles == null) {
            XueChengPlusException.cast("文件保存信息失败");
    ...
    @Override
    @Transactional
    public MediaFiles addMediaFilesToDb(Long commonId, String fileMd5, UploadFileParamsDto uploadFileParamsDto, String bucket, String objectName) {
        // 将文件信息保存到数据库
        ...
        if (mediaFiles == null) {
            ...
            // 机构id
            mediaFiles.setCompanyId(commonId);
            // 桶
    ...
    public RestResponse<Boolean> checkChunk(String fileMd5, int chunkIndex) {
        // 分块存储路径是: md5前面两个为子目录,chunk存储分块文件
        // 根据md5得到分块文件所在目录的路径
    ...
    public RestResponse uploadChunk(String fileMd5, int chunk, String localChunkFilePath) {
        ...
        // 将分块文件上传到minio
        boolean b = addMediaFilesToMinion(localChunkFilePath, mimeType, bucket_video, chunkFilePath);
        if (!b) {
    ...
    public RestResponse mergechunks(Long companyId, String fileMd5, int chunkTotal,
        ...
        // 指定合并后的objectName等信息
        ComposeObjectArgs composeObjectArgs = ComposeObjectArgs.builder().bucket(bucket_video).object(objectName) // 合并后的文件的objectName
     ...
     * @return void
     * @author ragnarok
     * @date 2023/7/11 00:06
     * @version 1.0.0
     */
    private void clearChunkFiles(String chunkFileFolderPath, int chunkTotal) {
        // Stream.iterate(0, i -> ++i).limit(chunkTotal).map(i ->
        // ComposeSource.builder().bucket(bucket_video).object(chunkFileFolderPath +
    ...
    /**
     * description 得到合并后的文件地址
     *
     * @param fileMd5 文件id的md5值
     * @param fileExt 文件扩展名
     * @return String
     * @author ragnarok
     * @date 2023/7/10 23:31
     * @version 1.0.0
     */
